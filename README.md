...
### 1-3-3. 워크스페이스는 **주제 단위**로 나눈다 (운영 원칙)

**대용량 문서와 소형 문서를 한 풀에 섞지 않는다.** 청크 수 비대칭 때문이다.

실측: 437KB 매뉴얼 1건이 전체 청크의 **97.1%** (1,131 / 1,160)를 차지했다.
`Max context snippets`(topN) 는 문서가 아니라 **청크** 기준이라, 청크를 1,100개 가진 문서가
상위 4개를 독차지할 구조가 된다.
...
| `manuals` | 장비 매뉴얼 | 2건 (CANopen, RS232) |

두 워크스페이스는 **설정이 동일**하다 (임베더 bge-m3 / 청크 512·80 / topN 4 /
History 2 / Query 모드 / 같은 System Prompt). 나눈 것은 문서 풀뿐이다.

분리 후 실측 — 질문이 각자의 워크스페이스에서 정확히 답을 찾는다:

| 워크스페이스 | 질문 | 결과 |
|---|---|---|
| `kdocs` | EQ-0219 차기 교정일·상태 | ✅ 표 행 원문 인용 |
| `manuals` | SDO Write 텔레그램 바이트 구조 | ✅ RS232 매뉴얼에서 인용 |

> 새 주제의 문서 묶음을 넣을 때는 **워크스페이스를 새로 만드는 것**을 먼저 고려한다.
> 한 워크스페이스에 다 넣고 topN 을 올리는 방식은 효과가 없었다 (`EVAL.md` 부록 D).

### 1-3-4. 질문 표현이 검색 결과를 바꾼다 (실측)

같은 대상을 물어도 **문서에 쓰인 용어를 쓰느냐**에 따라 결과가 갈린다:

| 질문 | 결과 |
|---|---|
| `Position Range Limit 오브젝트는 무엇에 쓰이는가?` | ✅ 정의 문장을 정확히 인용 |
| `0x607B의 정의는?` | ❌ "문서에서 확인되지 않습니다" |
| `0x607B object의 용도를 …` | ⚠ 같은 번호가 나오는 **예제 값** 청크를 잡아 부정확하게 답함 |

인덱스 번호만 쓰면 그 번호가 등장하는 **다른 문맥**이 걸린다.
**문서에 실제로 쓰인 명칭(`Position Range Limit`)을 함께 넣으면** 원하는 청크가 온다.
