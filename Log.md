...
## 2026-08-11 (3차) — 인용 기본값화 (System Prompt)

### 작업

1. `kdocs` System Prompt 를 문서 조회 5규칙으로 설정 (`openAiPrompt`, API 적용).
   **재조회로 저장 확인** — 257자 완전 일치, 규칙 1~5 전부 존재.
2. 회귀 3문항 → 2건이 "확인되지 않습니다". **기본 프롬프트 통제군을 함께 측정**한 결과
   **두 조건 결과가 완전 동일** → System Prompt 탓이 아님을 확정.
   따라서 "규칙 5 제거 재시도"는 **실행하지 않았다**(근거 없음).
3. 검색이 되는 질문으로 인용 동작 검증 → `원문 인용: "■ Negative limit (0x607B.01) = –2048"`
   형태로 규칙 2 준수 확인. **과도한 거부 없음.**

### 새로 드러난 문제 — 대용량 문서의 검색 독점

`EN_7000_05048` (437KB, CANopen 매뉴얼)이 전체 청크의 **97.1%** 를 차지(벡터 29 → 1,160).
`topN 4` 는 청크 기준이라 인용 4개가 전부 이 문서에서 나오고, 작은 문서 질문이 검색에서 밀린다.

**EVAL §5 에 적어둔 "실사용 규모 미검증" 한계가 현실화된 사례.**
문서화(워크스페이스 분리 권장)만 했고 **근본 대응은 하지 않았다.**

### 문서 반영

README §1-3-2(값·인덱스 3수칙) / §1-3-3(대용량 문서 실측) 신설,
QUICKSTART 3-6(System Prompt 전문·목적표·실측 예) 신설 + 질의 요령 ④ 추가.

### 미검증

대용량 문서 검색 독점의 근본 대응(문서별 균형 검색 / 워크스페이스 분리 자동화) —
후보만 기록, 전부 미실행.

---

## 2026-08-11 (4차) — 워크스페이스 주제 단위 분리

### 작업

1. `manuals` 워크스페이스 신규 — CANopen(05048) + **RS232(05052)** 매뉴얼 2건.
   05052 는 인박스에 이미 있었고 변환도 끝난 상태여서 인간 투입 대기 없이 진행.
2. `kdocs` — 소형 업무문서 5종으로 재구성 (매뉴얼 제거).
3. 두 워크스페이스 설정 **실조회 검증** → 전 항목 일치 (query/4/0.25/2/default/프롬프트 257자).
4. 분리 회귀 3건.

### ⚠ 직전 진단 정정

앞 회차에서 EQ-0219·차단 지연 실패를 **"대용량 문서의 검색 독점"** 으로 진단했는데 **틀렸다.**
`kdocs` 에는 `EN_7000_05048` **1건만** 임베딩돼 있었고 소형 5종은 **아예 없었다** — 문서 부재였다.
청크 비율(97.1%)만 보고 독점이라 단정한 것이 잘못이다. 원칙 자체는 유효하나
**"독점을 실측했다"는 주장은 철회**한다. 진짜 독점은 아직 관측하지 못했다.

### 회귀 결과

| WS | 질문 | 출처 | 결과 |
|---|---|---|---|
| kdocs | EQ-0219 차기 교정일 | 교정장비_관리대장 | **성공** (표 행 원문 인용) |
| manuals | 0x607B 정의 인용 | EN_7000_05048 | **실패** |
| manuals | SDO Write 바이트 구조 | EN_7000_05052 | **성공** (원문 인용 설명) |

라우팅은 정확. EQ-0219 성공의 원인은 독점 제거가 아니라 **없던 문서를 넣은 것.**

### 신규 실측 — 질문 표현이 검색을 바꾼다

`Position Range Limit 오브젝트는 무엇에 쓰이는가?` → 정의 문장 정확 인용 **성공**
`0x607B의 정의는?` → 실패 / `0x607B object의 용도` → 예제 값 청크를 잡아 부정확

인덱스 번호만 쓰면 다른 문맥이 걸린다. **문서에 쓰인 명칭을 함께** 넣어야 한다.

### 부수 발견

소형 문서 재업로드 시 `(붙임1)` 이 **front-matter 구판**으로 선택돼 깨끗한 판본으로 교체.
`custom-documents` 에 동일 제목 구판이 남아 있어 **제목만으로 고르면 위험**하다.

### 문서 반영

README §1-3-3(주제 단위 원칙)·§1-3-4(질문 표현), QUICKSTART 3-5 원칙 블록·질의 요령 ⑤.
