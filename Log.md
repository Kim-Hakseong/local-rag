...
## 2026-08-11 (5차) — BB-1 수정 + 온보딩 보강

### BB-1: 미설치 환경에서 2단계 사망

- 원인: `& cmd /c "where kordoc" 2>$null` — 리다이렉션이 PowerShell 쪽에 있었다.
  `where` 는 못 찾으면 stderr 를 내고, PS 5.1 이 이를 ErrorRecord 로 승격 → 스크립트 사망.
- **M6 실증이 못 잡은 이유**: 내 PC 는 kordoc 기설치라 **미설치 분기를 안 밟았다.**
- 수정: PowerShell 레벨 리다이렉션 **6곳 전부** cmd 내부(`2>nul`)로 이동.
  **`npm install` 도 같은 함정**이어서 함께 수정 — `where` 만 고쳤으면 다음 줄에서 또 죽었다.
- 부수 개선: npm 실패 시 전체 로그 경로·프록시 힌트 안내.

### 함께 고친 결함

7단계가 서버를 무조건 기동하려 해서, **이미 정상 구동 중이면 setup 이 실패**했다.
→ 리스닝 확인 후 이미 떠 있으면 **기동 생략, 응답 검증만**.

### 회귀 (미설치 상태에서 시작)

`npm uninstall -g kordoc` 실제 제거 후 실행 → 2단계에서 자동 설치(179 packages, 33s) →
**SETUP COMPLETE / exit 0 / 48.1초.** GV-1 PASS. kordoc 재설치 원복 확인.

### QUICKSTART 온보딩 보강 (동료 첫 설치 실측 반영)

- 워크스페이스 생성 직후 **문서 투입 전에** `Query` 모드 + History 2 설정 (Automatic 이면
  에이전트 라우팅으로 무응답 — 실측 2회)
- **Save and Embed 는 서버 기동 후에** — 8091 없으면 조용히 실패
- **"임베딩된 파일 목록 알려줘" 같은 메타 질문은 동작 안 함** — 문서 관리 화면에서 확인
- 재업로드 시 **동명 구판 주의** (실측: 1,509자 구판이 선택됨)
- 문제 해결 표에 3행 추가 (에이전트 라우팅 / 업로드했는데 검색 안 됨 / "정보 없다" 반복)

### 운영 규칙 신설

**검색 실패 진단의 1단계는 워크스페이스 임베딩 목록 실사.** 목록 확인 없이 원인을 단정하지 않는다.
(직전 오진의 재발 방지 — 청크 비율만 보고 "독점"이라 단정했으나 문서 부재였다.)
