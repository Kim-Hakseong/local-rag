...
> **운영 원칙: 워크스페이스는 주제 단위로 만든다.**
> 장비 매뉴얼 같은 **대용량 문서**와 보고서·대장 같은 **소형 문서를 한 곳에 섞지 않는다.**
> 검색은 문서가 아니라 **청크** 단위로 상위 4개를 고르는데, 수백 페이지 매뉴얼 하나가
> 청크의 97% 를 차지하면 소형 문서가 밀려난다 (실측). 예: `kdocs`(업무문서) / `manuals`(매뉴얼).
...
### ⑤ 인덱스·코드 번호만 쓰지 말고, 문서에 쓰인 **명칭**을 함께 넣는다

같은 대상인데 표현에 따라 결과가 갈린다 (실측):

| 질문 | 결과 |
|---|---|
| `Position Range Limit 오브젝트는 무엇에 쓰이는가?` | ✅ 정의 문장 정확히 인용 |
| `0x607B의 정의는?` | ❌ "확인되지 않습니다" |

번호만 쓰면 그 번호가 나오는 **다른 문맥**(예제 값 등)이 걸린다.
